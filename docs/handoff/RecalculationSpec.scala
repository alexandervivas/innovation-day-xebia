...
/** Spec-first test for CB-15a (RecalculationStrategy + FullReplay).
  *
  * Written BEFORE the engine exists: it defines the contract the implementation
  * must satisfy. All expected figures were verified independently with decimal
  * arithmetic.
  *
  * Conventions the engine must follow:
  *   - Installment = annuity on monthly rate (annualRate / 12), rounded HALF_UP to cents.
  *   - Interest accrues daily, actual/365, on outstanding principal only,
  *     kept at full precision and rounded HALF_UP to cents when allocated or reported.
  ...
  *   - Days past due: counted from the due date of the oldest installment not fully
  *     covered by repayment amounts allocated to interest + principal.
  *   - Only USER events go in; the engine derives accruals, late fees and status.
  *
  * Assumed domain API (to be created in corebanking.domain / corebanking.engine):
  *   final case class LoanTerms(principal, annualRate, termMonths, disbursementDate, graceDays, lateFee)
  *   enum UserEvent: Disbursement(id, amount, valueDate, bookingDate)
  *                   Repayment(id, amount, valueDate, bookingDate)
  *   final case class Allocation(fees, interest, principal)
  *   enum LoanStatus: Current, InArrears
  *   final case class LoanState(principalOutstanding, interestAccruedUnpaid,
  *                              lateFeesCharged, daysPastDue, status,
  *                              allocations: Map[String, Allocation])
  *   enum ChainStep: Reverse(targetId), Post(txId), Repost(originalId)
  *   final case class RecalcResult(before: LoanState, after: LoanState, chain: List[ChainStep])
  *   enum RecalcError: ValueDateBeforeDisbursement, PeriodClosed(periodStart), ...
  *   trait RecalculationStrategy:
  *     def stateAt(terms, events, asOf): LoanState
  *     def backdate(terms, events, newTx, systemDate, closedPeriods): Either[RecalcError, RecalcResult]
...
object RecalculationSpec extends ZIOSpecDefault:
  ...
  def spec = suite("FullReplay — LN-0042 backdated repayment")(
    ...
    test("rejects backdating into a closed accounting period") {
      val result = FullReplay.backdate(terms, history, backdated, systemDate,
        closedPeriods = List(d("2026-09-01")))
