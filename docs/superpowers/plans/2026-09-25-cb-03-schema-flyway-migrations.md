...
- All money columns use `NUMERIC(18,2)`; all rate columns use `NUMERIC(6,4)` (matches `0.0800` in the seed reference file). `accruals.amount` is the one deliberate exception (updated post-review to an unconstrained `NUMERIC` for exact precision — see `V1__schema.sql`'s header note and CLAUDE.md rule 2/`RecalculationSpec.scala`'s full-precision accrual contract).
