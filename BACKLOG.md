...
| ID | Story | Acceptance Criteria | Path | Status |
|---|---|---|---|---|
| CB-12 | system_clock + advance_date | `system_clock` table holds single row: `current_date`. `advance_date(days)` increments it. All subsequent reads use this clock, not server clock. `get_system_date()` read tool. | critical-path | in-progress |
