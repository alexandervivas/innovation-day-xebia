...
| CB-04 | Seed products | Flyway `V2__seed_products.sql` inserts: Savings product (annual interest, no term). 12-month Loan product (8% annual, annuity schedule, daily accrual). Both use `NUMERIC(18,2)` for money fields. | critical-path | in-review |
...
| ID | Story | Acceptance Criteria | Path | Status |
|---|---|---|---|---|
| CB-12 | system_clock + advance_date | `system_clock` table holds single row: `current_date`. `advance_date(days)` increments it. All subsequent reads use this clock, not server clock. `get_system_date()` read tool. | critical-path | todo |
