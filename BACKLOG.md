...
| CB-10 | Audit log | Every tool call (read + write) logged to `audit_log(id, tool_name, params, result, timestamp, dry_run_flag)`. `get_audit_log(start_time?, end_time?)` read tool to retrieve history. | critical-path | done |
